...
def setup_args(data_args, raw_datadict):
    ...
    text_columns = sorted(
        [c for c in data_columns if c.startswith("sentence")])[:2]
...
def setup_trainer(model_name_or_path, config_name, datadict, data_args, training_args, tokenizer, from_tf=False):
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        preds = np.squeeze(
            preds) if data_args.is_regression else np.argmax(preds, axis=1)
...
def evaluate_model(trainer, dataset, data_args, output_dir=None, stage="eval"):
    p = trainer.predict(dataset, metric_key_prefix=stage)
    predictions = np.squeeze(
        p.predictions) if data_args.is_regression else np.argmax(p.predictions, axis=1)
